...
public class ScratchFtpClientBackup {
  String host;
  int port;
  BufferedReader in;
  PrintWriter out;

  public ScratchFtpClientBackup(String host, int port) {
    this.host = host;
    this.port = port;
  }
  ...
  public void activeList() throws UnknownHostException {
    int port = 9999;

    String ip = InetAddress.getLocalHost()
        .getHostAddress();
    // 將.改為,
    String info = ip.replace('.', ',');
    int p1 = 9999 / 256;
    int p2 = 9999 % 256;
    info = info + "," + p1 + "," + p2;
    System.out.println(info);
    sendCommand("PORT " + info);
  ...
  public static void main(String[] args) {
    ScratchFtpClientBackup client = new ScratchFtpClientBackup(
        "ftp.nsysu.edu.tw", 21);
    try {
      client.connect();
      client.getWelcome();
